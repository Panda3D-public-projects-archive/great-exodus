...
class ProductionObjectFactory(object):
    ...
    def create_small_planet(self, stock_manager, productionTimerFactory):
        po = self.create_production_object( stock_manager.create_small_planet_stock(), self.small_planet_production)
    ...
    def create_small_factory(self, resource, stock_manager, productionTimerFactory):
        po = self.create_production_object( stock_manager.create_small_factory_stock(resource), [resource, ])
        productionTimerFactory.add_small_production_object(po)
        self.add_production_object_to_list(po)
    ...
    def create_production_object(self, stock, production_lines):
        po = ProductionObject(stock, production_lines)
        return po  
